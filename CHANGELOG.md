...
- **Headless legs are no longer declared complete while the engine is still answering.** The
  ...
  `session.status`: while the engine reports `busy` (or `retry`, provider backoff) and no tool
  call is live, flat output is treated as activity rather than silence, and the leg waits for
  its message to finalize.
