...
describe('evaluateOutputBudget — a budget is configured', () => {
  ...
  test('valid at/below the default, no catalog cache -> ok: the flag alone can only lower (K12)', () => {
    ...
    expect(row.status).toBe('ok');
    expect(row.message).toBe('budget 8000 — each leg reserves min(8000, its ceiling where one is known); no catalog cache, so no route has a known ceiling here (the engine clamps routes its own catalog knows; an unknown model receives 8000 as-is; an openai/ id carries no output reservation on a leg that resolves DIRECT — the Responses API request has no output-limit field, probe M5/M13/M22 — while the openrouter/openai/… form of the same model does, M1/M9; which gateway a leg takes is a launch-time decision this row cannot read)');
    expect(row.hint).toBeNull();
  });
  ...
  test('a direct openai alias route is listed as carrying no reservation at all and never counted as clamped (#218 PR 4, M5/M13/M22)', () => {
    ...
    expect(r.message).toContain('1 openai/ alias route (openai/gpt-5.6-terra) carries no output reservation on a leg that resolves DIRECT to that provider — the engine drives it through the Responses API, whose request has no output-limit field (probe M5/M13/M22); which gateway a leg takes is a launch-time decision this row cannot read (--gateway, routing.prefer, key presence), and the openrouter/openai/… form of the same model DOES carry the reservation (M1/M9)');
    // Named mutant "OPENAIOUTCOMEASSERTED" (council #235 r4, C6): restore the unconditional
    // "carries no output reservation at all … so the value does not apply there". The doctor
    // cannot see a future invocation's --gateway, so the id prefix partitions the LIST; it never
    // predicts the route a leg will take.
    expect(r.message).not.toMatch(/carries no output reservation at all/);
    expect(r.message).not.toMatch(/so the value does not apply there/);
  ...
  test('two direct openai alias routes are named together with the plural clause (#218 PR 4, M5/M13/M22)', () => {
    ...
    expect(r.message).toContain('2 openai/ alias routes (openai/gpt-5.6-terra, openai/gpt-5.3-codex) carry no output reservation on a leg that resolves DIRECT to that provider — the engine drives it through the Responses API, whose request has no output-limit field (probe M5/M13/M22); which gateway a leg takes is a launch-time decision this row cannot read (--gateway, routing.prefer, key presence), and the openrouter/openai/… form of the same model DOES carry the reservation (M1/M9)');
  });
