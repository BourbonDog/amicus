 ...
 * `last-notified.json` in `alias-notice-state/` (council #254 rounds 1–2) —
 * never in config.json; without a terminal on stdin, under --json, or for
 * --help, nothing happens and no state directory is created. The predicate
 * and the decision are unit-tested in tests/utils/alias-refresh-state.test.js /
 ...
 * `saveConfig` pretty-prints (2-space) while this fixture is compact, so ANY
 * config write changes the bytes — the assertion is load-bearing, not
 * decorative; `normalizeOnEntry` saves nothing for an already-normalized
 * fixture.
 */
...
beforeEach(() => {
  ...
  stateDir = path.join(dir, 'alias-notice-state');
  notifiedPath = path.join(stateDir, 'last-notified.json');
  spawnedPath = path.join(stateDir, 'last-refresh-spawned.json');
...
const lastNotified = () => JSON.parse(fs.readFileSync(notifiedPath, 'utf-8')).at;

describe('the exit hook, wired (#238 D5)', () => {
  test('a terminal, a finished command: the notice is the LAST thing on stderr; config.json stays byte-identical and last-notified.json lands in alias-notice-state/ (mutant UNWIRED)', () => {
    ...
    expect(fs.existsSync(spawnedPath)).toBe(false);   // the fixture catalog is fresh: no refresh spawned
  });
  test('a second run within the day is silent and does not re-stamp (Q5)', () => {
    run(['list'], { tty: true });
    const stamp = lastNotified();
    const r = run(['list'], { tty: true });
    expect(r.stderr).not.toContain('alias update');
    expect(lastNotified()).toBe(stamp);
  });
