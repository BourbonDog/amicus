...
async function runDebate(ctx, { provisionalRecord, tallyInput }) {
  ...
  // SEAT ID into the alias-space `judge` field, which reaches tally.js's
  // `v.judge !== f.raiser` (measured: basis {a:1,d:0} → tier Confirmed where the
  // alias spelling gives Singleton) and report.js's `byJudge[adj.judge]`.
