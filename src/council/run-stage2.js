 ...
 *
 * #257 R-X37: what is left here is the SHELL — bindStage2Seats, the bundle, the
 * wave launch and the leg loop. One judge's whole adjudication (artifact, parse,
 * relaunch/repair, notes, result row) moved VERBATIM to ./run-stage2-judge.js,
 * which this file stood at 298 of the 300-line gate to make room for.
 *
...
// #257 R-X37: the leg loop's BODY, moved out whole for the 300-line gate.
const { adjudicateJudgeLeg } = require('./run-stage2-judge');
...
async function runStage2(ctx, { reviews, labels, globalFindings, extraLabeled = [] }) {
  ...
  const extraRows = [];
  // #257 R-X37: the loop BODY is `run-stage2-judge.js :: adjudicateJudgeLeg`,
  // moved there VERBATIM because this file stood at 298 of the 300-line gate.
  // `env` is the scope that body used to read from; `repairSeq` rides inside it
  // because the `-q<N>` ids are sequential across the WHOLE wave rather than per
  // judge, and the helper advances the counter it finds there.
  const env = { bundle, judges, labels, parseCtx, judgeSeatOf, judgeResults,
    extraRows, repairSeq: 0 };
  for (const leg of (wave && wave.legs) || []) {
    // The abort return is the helper's own, already in this function's shape
    // (`{ aborted, judgeResults, extraRows }`) — returned as it stands.
    const abort = await adjudicateJudgeLeg(ctx, o, leg, env);
