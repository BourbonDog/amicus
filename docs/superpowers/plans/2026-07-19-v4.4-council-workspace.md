...
> `4ff7173`, so it already predates v4.0, v4.1 and v4.1.1 — and both v4.2 and v4.3 will land before it. By the time anyone
...
|---|---|---|---|
| A1 | In-process live-snapshot read API, cheap at ≤2 s cadence | v4.3 §4.3: the composed live doc IS `amicus_status`'s rollup, stamped `view:'live'`; callable in-process via `require('./mcp-server').handlers` (precedent `src/cli-handlers-status.js:62`); composition reads Surface-A snapshot files only | `electron/ipc-workspace.js` `workspace:get-live` → `src/workspace/live-normalize.js` |
| A2 | Per-leg live token/cost fields | v4.3 §4.1: `progress.json` gains `{schemaVersion, type:'progress', usage:{tokens, costReported}}`; §4.3: composed legs gain `usage` + read-time resolved cost. Absent fields → the UI renders `—` (degrade, not break) | seats panel (`live-model.js` `seatCells`), cost gauge live top-up |
...
| A4 | Liveness/staleness classified by the data layer, never GUI-invented | v4.3 §4.1 (staleness >60 s + `metadata.status==='running'` = stall candidate via existing `isStalled`), composed doc's crash detection inherited from `amicus_status` | `live-normalize.js` passes flags through; renderer only displays |
...
| A7 | Poll is a sanctioned transport; push is a later compatible swap | v4.3 §3.1 ("The contract is files + polling. No push, no IPC, no fs.watch"); `workspace:live-update` reserved as the upgrade seam (spec §4.5) | poll loops in `workspace-app.js`; no main→renderer push channels in v1 |
...
Read the merged v4.3 watch/status code and one of its tests to record the actual council composed-doc fields (where legs live, stage list field name, `view:'live'`, per-leg `usage`, stall/crash flags):
...
Expected: all green / clean / `All files within size limits` (record the suite counts — they are the regression baseline for every later task; the v3.2.3 spec-time baseline 301 suites / 3806 passed / 5 skipped is superseded by whatever v4.0+v4.3 left).
...
and `progress.json` (v4.3 §4.1 shape — raw `usage` totals):
...
git commit -m "feat(workspace): normalize the v4.3 composed live doc behind one seam" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"
```
...
1. Add `'ui'` to the boolean-flag list inside `isBooleanFlag(key)` (today line 120; the v4.3 merge will have its own additions — append `'ui'` to the same array).
...
git commit -m "chore(release): v4.4.0" -m "Co-Authored-By: Claude Fable 5 <noreply@anthropic.com>"
```
