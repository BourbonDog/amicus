...
8. **Rotted line citations in `src/headless.js` comments (pre-existing, found by Task 2):** the comments citing `:413`/`:417` and `:365` for where `sessionId` is assigned, and `headless.js:1295` inside `sessionStatusSafe`'s docblock for the poll loop's own status read, point at lines that moved long ago (`check-citations.js` proves only in-range) — and the Task 2 review (F6) found two more `headless.js:NNN` citations at ≈ line 137 and ≈ line 412 that Task 2's own insertions made 53–89 lines staler. Replace each with a symbol anchor: the assignment sites → "`sessionId` is assigned in `runHeadless` at the `createSession`/shared-session branches, well before this return"; the poll-loop read → "`runHeadless`'s poll loop (`if (mirror.output.length > 0)` … `getSessionStatus`)". Comments only; run `npx jest tests/no-output-backstop-wiring.test.js` afterwards to confirm nothing else moved; include `src/headless.js` in this task's commit.

9. **BACKLOG.md — a new section at the END of the file**, `## #251 item 1 — filed, not fixed (2026-09-18)`, three lines in the file's house style (`- [ ] …`, ≤ 100 columns, one fact each):
   - The poll loop's backstop block sits INSIDE the poll body's `try … catch (pollError)`, and `consecutivePollFailures` is reset at the top of every poll before that block runs — so a throw inside the block is swallowed and retried every poll (5 ms in tests, 2 s in CI) until the leg `--timeout`, and the leg then dies as a generic `timeout` with the `NO_OUTPUT_BACKSTOP:` diagnosis lost. Measured on this branch when `decideBackstopExtension` still threw on an out-of-range `retry.next` (W11's RED was a 20 s hang, not the RangeError). The known thrower is gone; the shape is not: move the backstop block out of the poll `try`, or count its failures toward the bail. (#251 item 1, Task 2 fix round)
   - `tests/no-output-backstop-wiring.test.js` runs ≈ 30 s (from ≈ 15 s): the new W1/W2/W4/W5/W6/W11/W12 pins wait on real 1–2 s windows by design. Accepted cost; a fake-timer rewrite of that describe would buy the time back. (#251 item 1)
   - `src/headless.js` still carries `headless.js:NNN` line citations in comments that `check-citations.js` proves only in-range; Task 4 fixed the ones its sweep found — any it did not are listed in the Task 4 report. (#251 item 1)
   - `src/utils/result-schema.js` is at 300/300 with TWO emit-when-set spreads packed onto one line of `buildRunResult`'s literal (`finish` and `backstop`) and a lazy `require` inside that hot literal; the next field to reach `buildRunResult` has nowhere to go without splitting the file (the run-result shape into its own leaf is the natural cut). (#251 item 1, Task 3)
   - The pre-commit hook regenerates `docs/architecture-map.md` AFTER the suite runs, so a commit that adds an export sees `npm test` fail on the stale map until `node scripts/generate-docs.js` is run by hand first — bit Tasks 1 and 3. Run the generator before the suite in the hook, or document the order in CONTRIBUTING. (#251 item 1, Task 3)
...
```bash
git add BACKLOG.md CHANGELOG.md docs/troubleshooting.md docs/council.md docs/configuration.md docs/usage.md src/headless.js skills/second-opinion/MODEL-NOTES.md .github/workflows/council-review.yml tests/scripts/council-review-workflow.test.js
