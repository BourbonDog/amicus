...
**Release-ritual note (for the PR body and the next cut):** Phase 2 adds no user-visible change until a pin moves; the ritual gains (a) `amicus aliases --review --owner` refusing outside the checkout / on a dirty tree / without a TTY, and (b) `models --check` showing the `newer sibling:` lines. **The D3 baseline session runs AFTER this PR merges**, on the owner's machine with real keys, from `main` with a clean tree: `node bin/amicus.js aliases --review --owner`, review every proposal (the T7 smoke previewed five: gemini → 3.8-flash, fable → 5.1 on openrouter only, qwen-flash → 3.8, minimax → m3, grok → 4.20), read `git diff src/utils/curated-pins.json`, and commit as `pins: D3 baseline — <summary> (YYYY-MM-DD)`. Whatever is accepted there is the new curated pin set the next release ships.

**Expect a test-expectation sweep in that commit — the pins are the truth, the literals follow; never bend a pin to a test.** This PR's own new tests read pins from the frozen `b803a2a` fixture or a synthetic document (whole-branch review fix wave), so they stay green. But Phase 1's and older suites combine the LIVE builders with current-pin literals and will redden on any accepted move — measured by the whole-branch reviewer: `tests/config.test.js`, `tests/curated-models.test.js` (e.g. "fallback pins the terra tier"), `tests/curated-models-gateway-routes.test.js`, `tests/sidecar/aliases-command.test.js` (`gemini → google/gemini-3.6-flash`), `tests/sidecar/models-command.test.js`, `tests/utils/alias-audit-shipped-defaults.test.js`, `tests/utils/alias-state.test.js`, `tests/utils/gateway-route-audit.test.js`, `tests/route-launch.test.js`, `tests/setup-ui*.test.js`, `tests/config-resolve.test.js`, plus `tests/curated-models-move.test.js`'s "live shipped file today" test (designed to die in that commit — delete it). Update each literal to the accepted id in the same commit; a family pin the picker did not offer (its `idPattern` admits shapes the same-tier comparator refuses — `models --check`'s family line now says so) is a hand edit of the JSON in that same session.
