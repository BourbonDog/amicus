...
stops degrading the run) and **`KEYRAW`** (**1** — drop the join key's `|| 'unknown'` fallback, so a
leg carrying no model name renders `'undefined'`). ⚠️ Re-measured a SECOND time after review
round 1 added the note-fallback test: guard-deletion 5 → **6**, `WHYSTALE` 1 → **2**. The pre-T5.5
numbers are DATED readings, not live values.
