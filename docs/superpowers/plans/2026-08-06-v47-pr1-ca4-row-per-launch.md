...
- **E6 (D4 correction, recorded at Task-7 review 2026-08-06, controller-ruled within the E2 pattern — owner veto open):** the ledger-join allowlist additionally includes **`redteam`** — the second-opinion skill's documented primary-seat role (MANUAL-ORCHESTRATION.md:147 vocabulary `council | redteam | claude`; COUNCIL-DESIGN.md:266 says red-team runs record to the ledger normally). Omitting it would silently rewrite red-team rows to fabricated `conformance:'clean'` — the never-fabricate violation. Consequence documented in-code and CHANGELOG-lined: free-form/custom role labels are now REJECTED by the ledger join by design (fail-closed).
...
- [ ] **Step 1 (RED):** ledger.test.js: non-primary rows must NOT overwrite a model's seat row — **RED coverage required for ALL FOUR producer classes** (Task-6 review amendment): `chair-attempt` (run-chair.js), `repair` (run-stages/run-stage2/run-chair), `superseded` (run-stages + debate), and the debate-born pair. Carry in VERBATIM the test deleted by Task 6's fix wave ('superseded/repair rows never overwrite a bench model ledger row either' — see task-6-report.md fix-wave section); extend the #83 judge pin's pattern for the rest. live-model tests: new-role rows produce no seat row; F37 case unedited. report.test.js: new-role rows render suffixed.
