...
> **Superseded in part (2026-09-04):** the P1 probe refuted fact 4 (a per-model `options.max_tokens` DOES reach the wire, case E1) and the engine reports kimi-k3's ceiling as 1048576, not fact 6's live-models.dev 943718. Several further sentences drafted here and in the task bodies below were overtaken by review and must not be read as the shipped contract: (a) the fill rule is **"empty or unusable"**, not "nulls only" — the shipped guard is `positiveCount(...) === null`, so a 0, a negative, a value below 1 and a non-number are filled too, and an all-`unknown` models.dev document is now a `bad-shape` failure rather than a successful enrichment; (b) the release note's "Nothing changes with `outputBudget` unset" was replaced by **"no request changes"** with `outputBudget` unset — the catalog rows themselves do change; (c) `fmtCeilingLine` words a failure by its `failure.reason` instead of always claiming models.dev was unreachable; (d) `httpGetText`'s contract grew — Task 1's "Interfaces" line and the Task 8 CHANGELOG draft's "the failure vocabulary … is unchanged" are both false as written. It also takes **`maxBytes`** (default 16 MiB) and adds a **`too-large`** failure reason, and it **follows at most two same-scheme (`https:`) redirects** under one deadline, keeping **only an allowlist** (`user-agent`, `accept`, `accept-language`) **on any cross-origin hop** so a provider key cannot be forwarded to another host (the deny-list this banner first named was replaced by that allowlist in council round 4, and `accept-encoding` left it in round 5). (e) the `Ceilings:` line and its counters drafted in Task 5 are superseded: the shipped line reports FOUR counters (`filled`, `alreadyKnown`, `unknown`, `stillMissing`), words `alreadyKnown` as "already complete" and decides it BEFORE the models.dev lookup, and it has two SKIP forms the draft has no wording for at all — `modelsDevCeilings: false` and "nothing to fill (no candidate row is missing a number)"; it lives in `src/sidecar/models-ceiling-line.js`, not in `src/sidecar/models.js` where the draft put it; (f) direct `anthropic/*` routes receive **no reservation descriptor at all** — `config.js :: buildProviderModels` registers `{}` for them regardless of `outputBudget`, because the engine ADDS the thinking budget to `max_tokens` on that route (rows H1/H3/H4) and that was measured only against a bare descriptor; PR 2 measures descriptor x budget and lifts the guard, and `openrouter/anthropic/*` is unaffected. The BACKLOG "v4.9.4 records" P1 entry is the measured record, and `CHANGELOG.md` plus `src/utils/model-ceilings-modelsdev.js` and `src/utils/http-get.js` carry the shipped wording; read those before this section.
