...
`OUTPUT_LENGTH` having streamed only reasoning, and the death was announced by the reason this PR
wrote, in `run.json`'s one heal degrade: `OUTPUT_LENGTH: the provider stopped at the max_tokens
...
- R-X48 **The debate note names a promoted retry from an explicit marker, whichever branch its leg
  took.** Cost: the clause can now name a seat whose retry left no row, so a reader correlating the
  prose against `runStats` finds one fewer row than aliases — which is the truth about the round
