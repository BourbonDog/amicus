...
- R-X37 **The Stage-2 judge leg-loop body lives in its own module, and the stand-down cause has ONE
  ...
  byte-identical; the price is one more file in `src/council/`. ⚠️ Round 5's R-X49 (§14) adds an
  eighth `why` sentence and a seventh stand-down state to that explicit set; the seven named here
  are unchanged.
- R-X38 **The Stage-1 dead-leg prose is bounded like the Stage-2 judge-death prose, at a cap
  ...
  clears a computable worst case (620 on the pinned rows, 660 over all legal inputs — round 4 wrote
  627, measured from a NEGATIVE count that R-X50 (§14) established was never a report), not a
  sample.
...
- R-X46 **A promoted repair never supersedes real text, and the note names whose retry it was.**
  ...
  note now says so, by alias and by kind. ⚠️ Round 4 named the retry from the ROWS, which left the
  double-promoted COMPLETE case (this ruling's own case ii) unnamed; R-X48 (§14) names it from a
  marker instead. The rows and the four forms are unchanged.
...
## 14. Addendum — council round 5 (2026-09-20)

Council round 5 on PR #270 (run `35514703539`, 2026-09-20 13:51:20Z → 14:08:37Z, 17 min 18 s,
$0.80 over 11 priced legs) returned "Fix these first" with **4 of 4 seats reviewed** and
`seatsReviewed.unverified: 1` (`council-r5/council-run/verdict.json`). Chair gemini-pro.

**The round demonstrated this PR's own subject on itself.** deepseek's FIRST Stage-1 leg died
`OUTPUT_LENGTH` having streamed only reasoning, and the death was announced by #218's reason as
released (the workflow installs `amicus@latest`, v4.13.0, and never runs this branch, which mints
the same string for this record), in `run.json`'s one heal degrade: `OUTPUT_LENGTH: the provider stopped at the max_tokens
reservation (finish 'length') and only reasoning was streamed, no answer text — 64000 reasoning /
0 output tokens; outputBudget is 64000 — raise outputBudget in config.json (docs/configuration.md,
...
Every `file:line` below is measured on `feab74a5`, the head carrying fixes H1, H2 and H3; the
table's middle column names the behaviour at `ad7a8e67`, the head round 5 reviewed.

| finding | at `ad7a8e67` | rule |
|---|---|---|
| C1 (major, gpt, 3-0) = A1 (minor, glm, 3-0) = D3 (minor, deepseek, 3-0) — one gap raised three ways: a promoted defence or re-vote whose relaunch comes back promoted **and complete** is never named by the `debate-degraded` note, though its row carries `promoted: true` | CONFIRMED: `run-debate.js@ad7a8e67:297` built `promotedRepairs` by FILTERING THE RETRY ROWS — `[...defenseResults.map(d => d.repairLeg), ...revoteRepairs].filter(r => r && r.promoted === true)` — so membership depended on which BRANCH the retry took. R-X46's supersede arm (`leg2 && (!isPromotedLeg(leg2) \|\| isPromotedLeg(leg))`, `run-debate.js:92` / `run-debate-revote.js:177`) is TRUE for a promoted relaunch of an already-promoted leg, so case (ii) superseded, `repairLeg` / `repairRow` stayed null, no row reached the aggregation and the clause was silent — while the symmetric case (a REAL wave-1 whose promoted repair completes, case iii) WAS named. Round 4 proved only the TIMED-OUT double-promoted case reachable, which takes the ROW branch, and named that one. The seat was then visible only through its row's machine `promoted: true` — which is what R-X36's rule ("a `promoted: true` row is never the ONLY record of itself") forbids. | R-X48: the note names EVERY retry that answered in its reasoning channel, whichever branch its leg took, from an EXPLICIT marker rather than from the rows. `runDefenseSolo` (`src/council/run-debate.js:91`) and `repairRevoteLeg` (`src/council/run-debate-revote.js:174`) mint `promotedRetry: { alias, kind }` whenever the RETRY leg is promoted — complete or not, superseding or not — with `kind` `relaunch` when the WAVE-1 leg was promoted (its retry was a relaunch with the ORIGINAL briefing, R-X33) and `repair` otherwise. The marker rides BOTH arms of the re-vote return (`run-debate-revote.js:178`, `:180`), accumulates in a third list beside the rows (`:226`, `:250`, returned at `:297`), is bound in `runDebate` (`src/council/run-debate.js:218`), and `:297` builds `promotedRepairs` from the markers alone, de-duplicated per `kind + '\0' + alias` (the TWO-CHARACTER source escape, never a raw NUL byte). The ROWS do not change: case (ii) still supersedes, its rebuttal/revote row still carries `promoted: true`, and `run-debate.js:238`'s `repairLegs` still reads the rows. `promotedRepairClause` (`src/council/run-debate-stage.js:72`) is BYTE-IDENTICAL and the four shipped forms are unchanged — only the SOURCE of the aliases moved. Named mutants DOUBLEPROMOTEDUNNAMED, PROMOTEDRETRYKINDLOST, REVOTEMARKERUNGUARDED. |
| C2 (major, gpt, 3-0; HQ2) — a promoted repair following a promoted judge's relaunch is not identified as a reasoning-channel failure in the Stage-2 note | CONFIRMED: a PROMOTED judge whose relaunch answered for REAL, whose text did not parse, and whose one LC-12 repair answered in ITS reasoning channel resolved to `'relaunch-unparseable'` (the stand-down resolution at `run-stage2-judge.js@ad7a8e67:188-192`, MEASURED — the conflating ternary is `:191`; the finding's own `location` string gives the wider `:183-207`, which also covers the block's comment and the `#202` case below it), whose `why` reads `the relaunch's answer did not parse after its one repair` — byte-identical to the ending where the repair answered for real and merely failed the schema. An operator tuning prompt budgets could not tell a model that cannot follow the JSON schema from one that produced no answer text block at all. | R-X49: a SEVENTH stand-down state, `'relaunch-repair-promoted'`. A second gate beside R-X36's records it — `src/council/run-stage2-judge.js:177`, `if (isPromotedLeg(leg) && attempts === 2 && isPromotedLeg(solo.leg)) { repairPromoted = 2; }` — the stand-down resolution forks on it (`:208-213`, `repairPromoted === 2 ? 'relaunch-repair-promoted' : 'relaunch-unparseable'`), and the note's promoted arm carries `repairPromotedAttempt` (`:234`), emit-when-set beside `relaunched` / `rescued` / `relaunchWaveId` / `usedWaveId` (`src/council/run-stage2-notes.js:195`). The `why`'s cause, byte-exact (`run-stage2-notes.js:163`): `relaunched once with the original briefing, and the relaunch's answer did not parse and its repair answered in its reasoning channel`. The shared prefix (`relaunched once with the original briefing, and `) and the surrounding `why` (`its own answer was its deliberation, not a judgement; <cause>; the deliberation itself was read by nobody (<tokenSplit>)`) are unchanged, as are `what`, `effect` and the thin-cross-review reason; no new `channel:` literal. `data.repairPromotedAttempt` is `2` — the only value that gate can record, since a promoted judge's attempt 1 IS the relaunch — and it is the SAME key R-X36's arm carries, so one key answers "which ask answered in the reasoning channel" on either path. R-X36's gate for a NON-promoted judge is unchanged beside it (`:169`, `!isPromotedLeg(leg) && isPromotedLeg(solo.leg) && repairPromoted === null`) and the two cannot collide: R-X36's excludes a promoted judge by construction. Every other arm's `data` is byte-identical, and `relaunch-unparseable` carries no `repairPromotedAttempt` — its absence is the discriminator from the other side. Named mutants RELAUNCHREPAIRPROMOTEDHIDDEN, SEVENTHARMDROPPED. |
| A3 (nit, glm, 3-0) + B1 (minor, qwen, 3-0) — the three `token usage not reported` renderers use two different PER-COUNT predicates, so one usage record renders inconsistently | CONFIRMED: `utils/output-length.js@ad7a8e67 :: formatOutputLengthReason` gated its counts on `Number.isFinite(t.reasoning) && Number.isFinite(t.output)`, while `promoted.js :: promotedFacts` and `run-retry-notes.js :: truncatedReviewNote` used `Number.isInteger(v) && v >= 0`. `{ reasoning: 1.5, output: 3 }` therefore rendered `1.5 reasoning / 3 output tokens` in the first home and `token usage not reported` in the other two, and `{ reasoning: -1, output: 3 }` likewise. R-X44(b) pinned the three homes equal on the LITERAL; nothing pinned the predicate. | R-X50: ONE per-count predicate in all three homes, `Number.isInteger(v) && v >= 0`. `formatOutputLengthReason` ADOPTED it (`src/utils/output-length.js:114-115`); `promotedFacts` (`src/council/promoted.js:173-174`) and `truncatedReviewNote` (`src/council/run-retry-notes.js:286`) already spelled it, byte for byte. The FIVE-count gate is UNCHANGED — `reportedTokens` (`promoted.js:158`, hand-spelled a second time at `output-length.js:56` because `utils/` must not require `council/`) still reads `Number.isInteger(v) && v > 0` across the five counts, so "a usage record with no positive count is not a report" stands exactly as R-X44(c) ruled it. Only the two per-count tests inside `formatOutputLengthReason` moved. A fractional row and a negative row are pinned EQUAL across all three renderers. Named mutant PREDICATEDIVERGED. |
| B2 (minor, qwen, 2-0-1) — reasons that can be `null`/`undefined` now pass through `boundReason` (`collapseExcerpt`) before the `\|\| 'no reason recorded'` fallback, and `collapseExcerpt`'s null-safety is not visible in the diff | **REFUTED by measurement, no code change.** `src/utils/text-sanitize.js :: collapseExcerpt` coerces before it touches anything: `:80` reads `String(text === undefined \|\| text === null ? '' : text)`, so a nullish reason becomes `''` and the `\|\|` fallback then fires exactly as it did before R-X42. Round 4 already pinned the four falsy inputs against the real function. The concern is a real readability cost of a helper whose contract lives one module away, and it is answered by the measurement rather than by a guard that would duplicate `collapseExcerpt`'s first line. |
| A2 (minor, glm, 3-0; HQ1) — the promoted-reasoning exclusion is a SINGLE gate: any future producer of `reviews[]` that bypasses `materializeReviews` silently reintroduces the leak | **Answered, no code change.** The only producers of `reviews[]` in the tree are `src/council/run-stages.js` — declared at `:160`, pushed at `:266`, fed by `materializeReviews` at `:81` and `:149` — and `src/council/run-retry.js:186`, which calls `materializeReviews` itself to build the usable set. There is no checkpoint or resume path that reconstructs it: `src/council/` holds no checkpoint or resume module, and `run.js` passes `s1.reviews` straight through (`:219`, `:257`, `:281`). See the design note below for why a future reader inherits the discriminator anyway. |
| D1 (major, deepseek, 1-0-2) — `mirror.promotedOutput.length` is read with no guard; if `promotedOutput` were ever `undefined` the spread would throw and the leg emit would die | **REFUTED by measurement, no code change.** `src/sidecar/conversation-mirror.js:47` initialises `promotedOutput: ''` on the state object, and its only two assignments are `:164` (`state.promotedOutput = ''`) and `:276` (`state.output = state.promotedOutput = state.reasoningOutput`, itself initialised `''` at `:35`). It is a string on every path, so `.length` at `src/headless.js:2129` cannot throw. The finding's own basis (1 agree, 2 neutral) reflects that the bench could not see the mirror in the diff; the answer is the measurement, not a guard. |
| D2 (minor, deepseek, 0-1-2, CONTESTED; HQ4) — a reasoning-only `OUTPUT_LENGTH` death yields a non-empty promoted stand-in yet never carries `promoted: true`, so one phenomenon has two representations | **By design, no code change — and the contest was right.** The death path returns before the promoted emit (`src/headless.js:2124-2129`: the emit is `...(mirror.promotedOutput.length > 0 ? { promoted: true } : {})` at `:2129`, and the comment above it at `:2124-2128` names the case in so many words — `the failed return above never carries it (a death has no deliverable to classify — an L2/L4 OUTPUT_LENGTH death has a non-empty stand-in and is still a death)`), so `promoted` is never on a `status: 'error'` leg. The SCHEMA says the same thing, so it is a published contract and not an implementation accident: `schemas/run.schema.json:98` ends `never on a leg whose status is 'error'`, and `schemas/council-tally.schema.json:116` repeats it. That is the fence this spec calls "never on an error leg" (spec R12, `:226`), and it is what keeps `promoted: true` meaning "a leg that RETURNED — complete, timed out or aborted — and delivered no deliverable". A death already has a louder record than a flag: its own reason names the cause in words — `only reasoning was streamed, no answer text` — which is precisely how this round's own deepseek leg was announced. A consumer counting no-answer outcomes reads the death reason for a dead leg and `promoted` for a leg that returned; giving the two one flag would make a `promoted` row mean two different things, which is the mistake HQ1 was asked about from the other direction. |

### The hard questions

The four questions are quoted from `council-r5/council-run/chair-output.md:32-35`, byte for byte
but for the soft wrapping.

1. **HQ1** — If a future feature or checkpoint-resume path reconstructs the `reviews[]` array
   without invoking `materializeReviews`, how will the system prevent the silent reintroduction
   of 165k-character reasoning blocks into the judge bundle?
   **Answer:** It will not need to, because the gate is not the only record of the fact. See the
   design note below.
2. **HQ2** — If a judge's relaunch yields real but unparseable text, and the subsequent repair
   yields promoted reasoning, the final state is `relaunch-unparseable`. How can an operator
   tuning prompt budgets distinguish between a model that cannot follow the JSON schema and a
   model that refused to provide an answer text block entirely?
   **Answer:** It could not, and that was the defect. R-X49 gives the second case its own state
   and its own sentence, and `data.repairPromotedAttempt` beside it — so the two endings are
   distinguishable in prose AND in `run.json`, not merely in the operator's inference.
3. **HQ3** — The `boundReason` formatter now passes potentially null or undefined failure
   reasons directly to `collapseExcerpt` before applying the fallback string. What happens to
   the run state if the elided sanitizer throws a type error on non-strings?
   **Answer:** It cannot: `collapseExcerpt` coerces on its first line (`text-sanitize.js:80`),
   nullish to `''` and everything else through `String()`. The `|| 'no reason recorded'`
   fallback then fires on the empty string exactly as before R-X42 (B2). Nothing reaches the
   sanitizer that it does not first coerce, so there is no run state to lose.
4. **HQ4** — The `promoted` flag is derived from `mirror.promotedOutput.length > 0`. If a
   provider stream fails with an `OUTPUT_LENGTH` death while exclusively streaming reasoning,
   does the resulting leg correctly preserve its death status, or does the non-empty stand-in
   inadvertently trigger promoted-leg processing?
   **Answer:** It preserves its death status. The death path returns BEFORE the promoted emit,
   so no `status: 'error'` leg ever carries `promoted` (spec R12), and the death is named in
   words instead — this round's own deepseek leg is the demonstration (D2).

### Design note — why a bypassed gate is not a silent leak (A2, HQ1)

`materializeReviews` is a filter, not the discriminator. The discriminator is `promoted: true`,
minted once at the mirror boundary (`src/headless.js:2129`) and written onto the LEG DOCUMENT as an
emit-when-true rider, declared in `run.schema.json`, the tally schema and `wave.schema.json`. Every
council reader that DECIDES on it tests it through one leaf predicate, `isPromotedLeg`
(`src/council/promoted.js:28`), which reads `leg.promoted === true` and nothing else; the rider
projections that merely COPY the flag onto rows and metadata spell that test inline — nine of them
on this head (`run-stats-entry.js:120`, `tally.js:197`, `leg-riders.js:44`, `debate.js:187`,
`run-debate.js:103` and `:141`, `run-debate-revote.js:295`, `sidecar/session-utils.js:113`,
`sidecar/fanout-leg.js:241`), and not one of them chooses anything. So a future producer of `reviews[]` that
bypassed `materializeReviews` would still be handed legs that CARRY the fact on disk: it inherits
the discriminator whether or not it inherits the filter, and the one line it must write is the same
line every other reader writes. That is a weaker guarantee than a type — nothing forces the new
reader to call it — but it is stronger than a gate, because the fact survives serialization,
resume, and any reader written years later against the documents rather than against the call
graph. Making it stronger still (a materialized-reviews type that cannot be constructed from a
promoted leg) is out of #257's scope and is not filed: the census, the seat-loss record and the
`reviewed` count would all still be the surfaces that caught it.

### ⚠️ The cap numbers moved, and round 4's were wrong

Round 4 (R-X43, §13) wrote **627** as the supremum of the longest reason amicus can mint, and
**173** as its headroom under the 800-character `MAX_LEG_ERROR_CHARS` cap. Both figures came from a
corpus row built with `{ reasoning: 1e308, output: -1e308 }` — and R-X50 establishes that a
negative count is not a report at all, so that input was NEVER legal. The row was measuring a
string the product cannot mint.

Re-measured on `feab74a5` by running the real formatters with the corpus's own pinned inputs:

| quantity | round 4 | round 5 (measured) |
|---|---|---|
| longest PINNED row (budget unset + a 96-digit PLAIN ambient flag, quoted twice) | 620 | **620** — unchanged |
| the same with a 500-character flag (the quote is capped) | 620 | **620** — unchanged |
| a 96-character NON-plain ambient flag | 605 | **605** — unchanged |
| headroom under 620 | 180 | **180** — unchanged |
| SUPREMUM over all legal inputs | 627 (from an ILLEGAL negative count) | **660** |
| headroom under the supremum | 173 | **140** |

The widest LEGAL count is `Number.MAX_VALUE`, whose decimal form is
`1.7976931348623157e+308` — 17 significant digits plus a three-digit exponent, **23 characters**,
beating a plain `1e20`'s 21 (anything at or above `1e21` flips to the much shorter exponential
form) and `Number.MAX_SAFE_INTEGER`'s 16. Used for BOTH counts it mints 660 characters — WIDER than
the illegal row's 627, because `1e308` prints as the six-character `1e+308`. R-X50 therefore did not
shrink the proven worst case; it moved the witness to a legal input that happens to print wider,
and the pinned rows (620, 605, 518, 517, 438, 398) never moved at all, because every one of them
passes `{ reasoning: 32000, output: 0 }` — legal under both the old predicate and the new. The
corrected figures are carried by `src/council/run-retry-notes.js`'s `MAX_LEG_ERROR_CHARS` docblock,
`tests/council/run-retry-notes.test.js`, `CHANGELOG.md`, `docs/troubleshooting.md` and §12/§13
above.

Rulings, each with its cost if wrong:
- R-X48 **The debate note names a promoted retry from an explicit marker, whichever branch its leg
  took.** Cost: the clause can now name a seat whose retry left no retry row, so a reader correlating the
  prose against `runStats` finds one fewer retry row than aliases — which is the truth about the round
  (the retry superseded), and the superseding row still carries `promoted: true`. The rows, the
  four forms and `promotedRepairClause` are unchanged, so every non-promoted round's `why` is
  byte-identical.
- R-X49 **A promoted judge's promoted repair after a real relaunch is a stand-down state of its
  own.** Cost: one more `standDown` value for a consumer enumerating them — `data` is emit-when-set
  so every pre-ruling record is byte-identical, and `relaunch-unparseable` narrowed to the case its
  sentence always described.
- R-X50 **One per-count predicate in all three token homes.** Cost: a fractional or negative count
  now reads `token usage not reported` in the `OUTPUT_LENGTH` reason where it used to render a
  number. No provider reports one; the change is observable only on a hand-edited artifact, and the
  measured worst case moved from an illegal 627 to a legal 660, still 140 under the cap.
