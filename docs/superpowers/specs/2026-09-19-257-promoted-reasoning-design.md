...
| A1 (confirmed minor, glm; raised at round 2 as B1 and filed then) — the Stage-1 dead-leg prose quoted `leg.error` RAW | the five Stage-1 prose sites interpolated provider-controlled `leg.error` verbatim into the `why`, while the analogous Stage-2 judge-death prose had been bounded with `collapseExcerpt(leg.error, 200)` since #219 | R-X38: all five bound their error text with `collapseExcerpt(…, MAX_LEG_ERROR_CHARS)` — `src/council/run-stages.js:127` (skipped leg) and `src/council/run-retry-notes.js:185` (still-dead first leg), `:225` (the `retryCause` fragment), `:239` and `:263` (`ff.reason`). The cap is **800**, defined and exported at `run-retry-notes.js:56`. **400 and 200 were REFUTED by measurement**: the cap bounds provider NOISE and must pass every reason amicus itself MINTS whole, and the longest minted reason today is **518** characters (`headless.js :: formatNoOutputBackstopReason`, caller-set + extended + every clause), with the `OUTPUT_LENGTH` remedy at **517**, a plain-ambient-flag `OUTPUT_LENGTH` at **438** and a non-extended backstop at **398** — three of nine over 400. 800 clears the longest by **282**, more than the 200-char engine-log excerpt that same format already embeds. Re-measured on this head against the real formatters with the pin's own inputs. The machine fields (`data.reason`, `ff.reason` as minted, `verdict.json :: seatLoss.reason`) stay RAW by design. Named mutants DEADLEGPROSERAW, RETRYPROSERAW, MINTEDREASONTRUNCATED. |
...
- R-X38 **The Stage-1 dead-leg prose is bounded like the Stage-2 judge-death prose, at a cap
  ...
  verbatim bytes. ⚠️ No finite cap is a PROOF while `formatOutputLengthReason` interpolates the
  operator-controlled `OPENCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX` verbatim
  (`src/utils/output-length.js:83-84`); the pin is a tripwire on drift, and the guarantee at the
  formatter is FILED — round 4 (R-X43) bounded the last unbounded input; the cap now clears a
  computable worst case of 620, not a sample.
