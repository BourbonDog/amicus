...
| D2 (minor, deepseek, 0-1-2, CONTESTED; HQ4) — a reasoning-only `OUTPUT_LENGTH` death yields a non-empty promoted stand-in yet never carries `promoted: true`, so one phenomenon has two representations | **By design, no code change — and the contest was right.** The death path returns before the promoted emit, so `promoted` is never on a `status: 'error'` leg; that is the fence this spec calls "never on an error leg" (spec R12), and it is what keeps `promoted: true` meaning "a leg that COMPLETED and delivered no deliverable". A death already has a louder record than a flag: its own reason names the cause in words — `only reasoning was streamed, no answer text` — which is precisely how this round's own deepseek leg was announced. A consumer counting no-answer outcomes reads the death reason for a dead leg and `promoted` for a complete one; giving the two one flag would make a `promoted` row mean two different things, which is the mistake HQ1 was asked about from the other direction. |

### The hard questions

1. **HQ1 — if a future feature or checkpoint-resume path reconstructs `reviews[]` without invoking
   `materializeReviews`, how will the system prevent the silent reintroduction of 165k-character
   reasoning blocks into the judge bundle?** It will not need to, because the gate is not the only
   record of the fact. See the design note below.
2. **HQ2 — if a judge's relaunch yields real but unparseable text and the subsequent repair yields
   promoted reasoning, the final state is `relaunch-unparseable`. How can an operator tuning prompt
   budgets distinguish a model that cannot follow the JSON schema from one that refused to provide
   an answer text block entirely?** It could not, and that was the defect. R-X49 gives the second
   case its own state and its own sentence, and `data.repairPromotedAttempt` beside it — so the two
   endings are distinguishable in prose AND in `run.json`, not merely in the operator's inference.
3. **HQ3 — `boundReason` now passes potentially null or undefined failure reasons directly to
   `collapseExcerpt` before applying the fallback string. What happens if the sanitizer throws a
   type error on non-strings?** It cannot: `collapseExcerpt` coerces on its first line
   (`text-sanitize.js:80`), nullish to `''` and everything else through `String()`.
   The `|| 'no reason recorded'` fallback then fires on the empty string exactly as before
   R-X42 (B2).
4. **HQ4 — `promoted` is derived from `mirror.promotedOutput.length > 0`. If a provider stream
   fails with an `OUTPUT_LENGTH` death while exclusively streaming reasoning, does the leg preserve
   its death status, or does the non-empty stand-in trigger promoted-leg processing?** It preserves
   its death status. The death path returns BEFORE the promoted emit, so no `status: 'error'` leg
   ever carries `promoted` (spec R12), and the death is named in words instead — this round's own
   deepseek leg is the demonstration (D2).
...
council reader tests it through one leaf predicate, `isPromotedLeg` (`src/council/promoted.js:28`),
which reads `leg.promoted === true` and nothing else. So a future producer of `reviews[]` that
