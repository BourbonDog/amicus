...
**Change.** Record the outcome of each poll's `getSessionStatus` as `lastSdkStatus ∈ {'busy','idle','other','unavailable'}`. The `!assistantFinished` branch of the stable-idle gate (threshold `stableIdlePolls`) may fire only when `lastSdkStatus !== 'busy'`. While busy, do not count the poll toward that exit; emit one debug line per deferral (`Idle heuristic deferred: SDK busy`, with `stablePolls`) so a trace shows the veto happening. The `assistantFinished` branch (threshold `stableFinishedPolls`, message already finalized) is unchanged. The `mirror.output.length > 0` arming guard is unchanged — with the veto in place, narration arming is harmless.
