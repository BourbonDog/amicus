...
Amicus is at **v4.9.4** (2026-09-07). Each 4.x rev below leads with the benefit, not the
plumbing; the patch releases since v4.9.0 carry no section of their own, because each corrected a
defect rather than adding scope — where v4.9.4 added a surface (`--thinking` refusals, the
`output-budget` doctor row, the wire probe) it did so to make an existing promise true, not to
widen it. See `CHANGELOG.md` for what each one contained.

**Status:** v4.0 through **v4.9.0** have **shipped**, plus the v4.9.1–v4.9.4 patch releases —
