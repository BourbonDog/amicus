...
- **When a leg hits it, the run says so.** The engine records `finish: 'length'` on the leg's
  ...
  haiku), so a catalog row whose ceiling exceeds the engine's is harmless.
