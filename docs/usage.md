...
| `SILENT` (`accepted-but-silent`) | `SILENT: probetest -> anthropic/claude-opus-4-8 — NO_OUTPUT_BACKSTOP: … (accepted but not serving)` | The probe saw no output at all for its 30 s backstop window. ⚠️ The `accepted-but-silent` label names the CLASSIFICATION, not a proven fact about the endpoint: a fired backstop shows only that nothing arrived in the window, which a stalled gateway or a dropped connection produces just as readily as a model that accepted and then said nothing. The window is (shorter than the ordinary 300 s default, and not tunable) — the exact "listed but not actually serving" failure this check exists to catch. |
