...
| `--agent <Plan\|Build>` | Escape hatch: run every leg on the engine's own agent instead of the council agents (no allowlist). Cannot be combined with `--tools`. Over MCP: the `agent` param. |
