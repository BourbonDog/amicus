...
| `output-budget` **(#218)** | `outputBudget` is a positive integer and says which alias routes have a known catalog ceiling; whether an ambient `OPENCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX` is honoured or overridden; flags a value in either place that is not a plain positive integer (the engine falls back to 32,000 without a word on those) and a budget that takes at least half of a route's context window | warn |
