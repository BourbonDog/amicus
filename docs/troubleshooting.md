...
Three trailing clauses are the exceptions, and none is the mechanism's inference:
...
The three clauses are independent: any can appear without the others, and the skew clause is **not** conditional on the log read succeeding. Everything before them is still only "silence past the deadline".
