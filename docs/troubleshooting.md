...
- ` — window extended once from 480s to 912s at 481s on session busy`, ` — not extended: the window is already at the leg cap`, or ` — not extended: the engine schedules its next attempt at <ISO instant>, past the extended window` (since the next minor after 4.12.0, #251 item 1) says what the backstop DID with that answer: a `busy` or `retry` session gets one more window (doubled, clamped strictly below the leg `--timeout`), and this clause appears only when there was a decision to report — a kill on `idle`, on an engine arm this code does not know, or on a probe that could not answer carries no such clause at all, byte for byte the string 4.12.0 built. The head of the message names the window **in force at the kill**, so after an extension it reads the extended number (`912s`, not `480s`) and this clause is what says where that number came from. The leg's document carries the same fact as a `backstop` record — on the leg's `metadata.json`, and on its leg entry in the wave's `wave.json` (present only when the backstop fired); a council run's `run.json` has no leg entries at all and carries the death report only as text, inside `degrades[]`.
