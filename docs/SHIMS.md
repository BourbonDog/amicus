...
Two rows are **not** removals — they are re-scoped or were never a shim to
begin with — and are called out explicitly below the table.
...
| Public API | `src/index.js` | `startSidecar`/`listSidecars`/... aliases | REMOVED in v2.0.0 | These were never separately-exported deprecated aliases — `startSidecar` etc. are (and remain) internal require-bound identifiers from `src/sidecar/*.js`; the only ever-*exported* public names are the canonical `startAmicus`/`listAmicus`/`resumeAmicus`/`continueAmicus`/`readAmicus`. If external code was importing a `*Sidecar`-named export from `amicus`'s package root, it was never valid — use the `*Amicus` names. |
...
| Fold-marker constant | `src/headless.js` `FOLD_MARKER`/`COMPLETE_MARKER` exports | bare `[SIDECAR_FOLD]` literal for callers with no nonce context (real runs use the per-run nonced `[SIDECAR_FOLD:<nonce>]`; the detector never accepts the bare form — see [docs/architecture.md](./architecture.md)) | **KEEP** — deliberate wire-token continuity, not a compat shim, not scheduled for removal (B38 keeps it) | No action needed. |
...
- `grep -r "SIDECAR_" src/` → no hits outside comments/docs referencing this history.
...
- `c3de5bf` — dead `sidecar-config-hash` comment-parse tolerance (in `config.js`; the skill-text acceptance instruction is fixed by this docs sweep)
