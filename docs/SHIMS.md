...
- `grep -r "SIDECAR_" src/` → every hit is either the `[SIDECAR_FOLD]`/`[SIDECAR_FOLD:<nonce>]` fold-marker constant (`src/headless.js`, `src/prompt-builder.js`, `src/utils/fold-marker.js` — deliberately kept, see the Fold-marker constant row above) or a comment in `src/sidecar/interactive-process.js` referencing that same constant. No `SIDECAR_*` env var is read or written anywhere in `src/`.
