...
| `findings[].unattributedPeerDrops` | **v4.8**, optional, integer `> 0` only — a count of the peer votes the alias fallback excluded from `basis` while exactly one side of the pair (the finding's `raiserSeat` or the vote's `seat`) carried a seat id and the other did not. Those are the votes nobody can attribute: a seat-less vote sharing the raiser's alias cannot be told apart from the raiser's own, so excluding it is the safe call **and** may be discarding a real twin's signal. Emitted only when `> 0`, so a document with no ambiguous drop is byte-identical to a pre-v4.8 one. ⚠️ **This field announces the drop; it does not change it.** The vote it counts is still excluded from `basis` — deliberately, by owner ruling R2 — and this release does not close that undercount (tracked as SI-22.1 / SI-22.2). It is also not yet rendered anywhere: as of v4.8 T2.3 the count reaches only `tally.json` — `verdict.js`'s findings literal does not copy it, so it never reaches `verdict.json`, and no renderer (`council report`, the Workspace matrix, the defense brief) displays it. |
