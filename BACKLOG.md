...
- [x] **#218 PR 4 — the effort lever beside the budget on every curated route, measured
  (2026-09-06).** Twenty-four cases (M0–M22) were added to `scripts/probe-max-tokens.js` with the
  case fields the M group needed: `dump: 'all'` (M0 starts ONE engine and dumps every curated
  route), `dumpOnly`, `waitKnown` (poll `/config/providers` until the model is known), `update` +
  `dumpAfter` (a `PATCH /config` sent to the RUNNING engine, the models re-dumped once it lands,
  and the sandbox files the call created or changed), and `viaAmicus` + `variant` + `outputBudget`,
  which send through amicus's own `sendPrompt` so the shipped validator is measured against the
  live engine. `want` gained a `refused` half (a row whose expectation is a typed refusal before
  the wire, M19/M20), an `update` half (the PATCH's status and whether it wrote `config.json` into
  the engine's cwd, M3/M4/M8/M11) and `reasoning: 'any'` (an effort field must be PRESENT, of any
  shape — M9, below). The whole 61-case matrix was run under one sandbox (engine 1.18.15 / sdk
  1.18.15 / server 1.18.15; 61 started, 61 closed): 59 matched, F3 and M0 recorded, nothing moved
  on the wire rows A–L5.

  What the M rows measured, as the plan's eight rules. (1) `variant` is the effort lever and
  `reasoning` never was (F1/F2, M1, M12, M18). (2) A variant the model does not declare is a silent
  no-op the engine still echoes — F3 on a known model, M7 on a model whose `variants` map is empty —
  so the artifact lies unless amicus refuses first (M19). (3) The engine's catalogue is the
  declaration and it can be late (M0 vs M12; the cache mechanism below). (4) A variant moves the
  reservation on exactly one shape: `thinking {type: enabled, budgetTokens: N}` on the direct
  Anthropic route adds N on top (M2: 24000 + 16000 = 40000) and the sum is clamped to the model's
  ceiling (M21: 64000), while OpenRouter (M1, M9), direct Google (M15), direct DeepSeek (M16) and an
  adaptive Anthropic variant (M10b) all leave the budget in force. (5) N is the engine's, not a
  formula amicus can run: `anthropic/claude-opus-4-5` declares 16000 for low, medium AND high with
  no max, haiku declares 16000/31999, and only the post-spawn dump carries it (M0). (6) Nothing
  changes a descriptor after the spawn — `PATCH /config` is accepted (200), changes neither what the
  engine serves nor what `GET /config` reports, and writes a merged `config.json` into the engine's
  cwd (M3 created it; M4 and M11 rewrote it), and a body missing `limit.context` is refused 400
  BadRequest, writing or changing no config file (the engine's SQLite log moved, as on every
  request) and the engine still serving (M8); the exact fit is proven on the wire (M17: descriptor
  8000 + flag 24000 + high → 24000) but needs N before the spawn. (7) The direct `openai` provider
  carries no output reservation at all — the Responses body has no `max_output_tokens` with or
  without a descriptor and the flag, for `gpt-5.6-terra` and `gpt-4o` alike (M5/M13/M22) — and the
  engine sends `reasoning.effort medium` there by itself when no variant is given (M13). (8) The
  vocabulary the curated routes declare between them is `none, minimal, low, medium, high, xhigh,
  max` (M0); the union of declared levels across the 36 models in the dump filed below is exactly
  those seven.
...
| E1 | options.max_tokens 4096 | 4096 — options.max_tokens reaches the wire (measured 2026-09-04; the plan predicted "dropped") | — | /api/v1/chat/completions | 4096 | — | — | 204 | length | — | — | 5/1/0 | text |
| E2 | options.reasoning {effort:low} | reasoning effort low on the wire | — | /api/v1/chat/completions | 32000 | {"effort":"low"} | — | 204 | length | — | — | 5/1/0 | text |
| F1 | amicus sendPrompt with the pre-PR-4 `reasoning` option (never a prompt field; ignored since PR 4) | NO reasoning on the wire | — | /api/v1/chat/completions | 32000 | — | — | 204 | length | — | — | 5/1/0 | text |
| F2 | prompt variant 'low' (kimi: low, high, max) | reasoning effort low | — | /api/v1/chat/completions | 32000 | {"effort":"low"} | — | 204 | length | low | — | 5/1/0 | text |
| F3 | prompt variant 'medium' (kimi has no medium) | record: silent no-op or error | — | /api/v1/chat/completions | 32000 | — | — | 204 | length | medium | — | 5/1/0 | text |
| F4 | prompt variant 'medium' (qwen has medium) | reasoning effort medium | — | /api/v1/chat/completions | 32000 | {"effort":"medium"} | — | 204 | length | medium | — | 5/1/0 | text |
| H1 | direct anthropic haiku {} | 32000 | — | /v1/messages | 32000 | — | — | 204 | length | — | — | 5/1/0 | text |
...
| L4 | direct anthropic haiku {} + variant 'high' — thinking block, no text, stop_reason max_tokens (usage output 24000) | finish 'length', 24000 output / 0 reasoning (Anthropic reports no split), a reasoning part and no text part | — | /v1/messages | 48000 | — | {"type":"enabled","budget_tokens":16000} | 204 | length | high | — | 5/24000/0 | reasoning |
| L5 | direct anthropic haiku limit.output 70000 + env 100000, no variant | 64000 if the engine clamps a descriptor above its own ceiling with no variant in play; 70000 if only a thinking sum is clamped (K10) | 100000 | /v1/messages | 64000 | — | — | 204 | length | — | — | 5/1/0 | text |
| M0 | dump only: the variants every curated route declares (bare descriptors, one engine) | record: each model's `variants` map as the engine reports it | — | — | — | — | — | — | — | — | — | — | — |
| M1 | kimi limit.output 8000 + env 8000 + variant 'low' (the shipped budget-8000 shape with an effort) | 8000 + reasoning effort low — on OpenRouter an effort does not move the reservation (F2 showed it at 32000) | 8000 | /api/v1/chat/completions | 8000 | {"effort":"low"} | — | 204 | length | low | — | 5/1/0 | text |
| M2 | direct anthropic haiku limit.output 24000 + env 24000 + variant 'high' (the shipped budget-24000 shape with a thinking variant, NO fit) | 40000 = 24000 + 16000 — the budget is overshot by the variant's budget (K2/K11 at a different point) | 24000 | /v1/messages | 40000 | — | {"type":"enabled","budget_tokens":16000} | 204 | length | high | — | 5/1/0 | text |
| M3 | direct anthropic haiku limit.output 24000 + env 24000, PATCH /config limit.output 8000, then variant 'high' | 40000 — PATCH /config is accepted (200) and changes nothing the engine serves; it wrote config.json into the engine cwd | 24000 | /v1/messages | 40000 | — | {"type":"enabled","budget_tokens":16000} | 204 | length | high | — | 5/1/0 | text |
| M4 | kimi {} then PATCH /config limit.output 4096, no variant | 32000 — the runtime update is accepted (200) on the OpenRouter provider too and changes nothing the engine serves; config.json in the engine cwd is rewritten | — | /api/v1/chat/completions | 32000 | — | — | 204 | length | — | — | 5/1/0 | text |
| M5 | direct openai gpt-5.6-terra {} + variant 'high' | no reservation at all on /v1/responses — the Responses body carries no output limit, and reasoning {effort: high, summary: auto} carries the level | — | /v1/responses | — | {"effort":"high","summary":"auto"} | — | 204 | — | high | APIError | 0/0/0 | (none) |
| M6 | direct google gemini-3.6-flash {} + variant 'high' | generationConfig.maxOutputTokens 32000 with thinkingConfig {includeThoughts, thinkingLevel: high} — the Google route carries the reservation and the level together | — | /v1beta/models/gemini-3.6-flash:streamGenerateContent?alt=sse | 32000 | — | {"includeThoughts":true,"thinkingLevel":"high"} | 204 | — | high | APIError | 0/0/0 | (none) |
| M7 | custom unknown model {} + variant 'high' (declares no variants) | 32000 with nothing added — a model whose variants map is empty takes the variant as a silent no-op, F3 on a model the engine does not know | — | /v1/chat/completions | 32000 | — | — | 204 | length | high | — | 5/1/0 | text |
| M8 | direct anthropic haiku {} then PATCH /config limit {output: 8000} with NO context (the ConfigInvalidError trap, at runtime) | 32000 — the bad runtime update is refused (400 BadRequest, missing limit.context) and writes or changes no config file; the running engine keeps serving | — | /v1/messages | 32000 | — | — | 204 | length | — | — | 5/1/0 | text |
| M9 | openrouter anthropic/claude-haiku-4.5 {} + variant 'high' (the catalogue-dependent effort field is recorded, not pinned) | 32000 on both catalogues — the effort field depends on which catalogue the engine had: the bundled one declares high as reasoning.max_tokens 16000, the live one as reasoning.effort high; either way the reservation is untouched (plan rule 4) | — | /api/v1/chat/completions | 32000 | {"effort":"high"} | — | 204 | length | high | — | 5/1/0 | text |
| M10 | direct anthropic claude-sonnet-5 {} + variant 'high' (an ADAPTIVE-thinking variant: effort, no budgetTokens) | 32000 — an adaptive variant adds nothing; thinking {type: adaptive, display: summarized} with the effort carried under output_config | — | /v1/messages | 32000 | {"output_config":{"effort":"high"}} | {"type":"adaptive","display":"summarized"} | 204 | length | high | — | 5/1/0 | text |
| M10b | direct anthropic claude-sonnet-5 limit.output 8000 + env 8000 + variant 'high' (the shipped budget-8000 shape with an adaptive variant) | 8000 — the budget holds under an adaptive variant, which adds nothing to the reservation | 8000 | /v1/messages | 8000 | {"output_config":{"effort":"high"}} | {"type":"adaptive","display":"summarized"} | 204 | length | high | — | 5/1/0 | text |
| M11 | direct anthropic haiku limit.output 24000 + env 24000, PATCH /config with the FULL spawn config (limit.output 8000), then variant 'high' | 40000 — a FULL-config PATCH is inert too (200); GET /config still reports the spawn value and config.json in the engine cwd is rewritten | 24000 | /v1/messages | 40000 | — | {"type":"enabled","budget_tokens":16000} | 204 | length | high | — | 5/1/0 | text |
| M12 | openrouter qwen3.8-max-0902 {} — wait for the engine's catalogue to know it, then variant 'medium' | 32000 with reasoning effort medium — the startup refresh made a model newer than the bundle known within 36 ms on a one-model engine and the variant then landed | — | /api/v1/chat/completions | 32000 | {"effort":"medium"} | — | 204 | length | medium | — | 5/1/0 | text |
| M13 | direct openai gpt-5.6-terra limit.output 8000 + env 8000, no variant | no reservation even with a descriptor and the flag — and the engine sends reasoning {effort: medium, summary: auto} by itself when no variant is given | 8000 | /v1/responses | — | {"effort":"medium","summary":"auto"} | — | 204 | — | — | APIError | 0/0/0 | (none) |
| M14 | direct deepseek deepseek-v4-pro {} + variant 'high' | 32000 with reasoning_effort high — the direct DeepSeek route carries the reservation and the level on chat/completions | — | /v1/chat/completions | 32000 | high | — | 204 | length | high | — | 5/1/0 | text |
| M15 | direct google gemini-3.6-flash limit.output 8000 + env 8000 + variant 'high' | 8000 — the budget holds on the Google route with a variant in play; thinkingLevel high adds nothing on top | 8000 | /v1beta/models/gemini-3.6-flash:streamGenerateContent?alt=sse | 8000 | — | {"includeThoughts":true,"thinkingLevel":"high"} | 204 | — | high | APIError | 0/0/0 | (none) |
| M16 | direct deepseek deepseek-v4-pro limit.output 8000 + env 8000 + variant 'high' | 8000 — the budget holds on the direct DeepSeek route with a variant in play; reasoning_effort high adds nothing on top | 8000 | /v1/chat/completions | 8000 | high | — | 204 | length | high | — | 5/1/0 | text |
| M17 | direct anthropic haiku limit.output 8000 (= 24000 − the high variant's 16000) + env 24000 + variant 'high' — the FITTED shape | 24000 = min(8000, 24000) + 16000 — a descriptor lowered by the variant's budget lands the sum exactly on the budget | 24000 | /v1/messages | 24000 | — | {"type":"enabled","budget_tokens":16000} | 204 | length | high | — | 5/1/0 | text |
| M22 | direct openai gpt-4o limit.output 8000 + env 8000, no variant (a chat-completions-era id: does the reservation appear on THAT path?) | no reservation on a chat-completions-era id either — the whole direct openai provider goes through /v1/responses, which scopes the M5/M13 finding to the provider, not to one id | 8000 | /v1/responses | — | — | — | 204 | — | — | APIError | 0/0/0 | (none) |
| M18 | amicus sendPrompt variant 'low' on kimi (declared) | reasoning effort low — F2 through the shipped path | — | /api/v1/chat/completions | 32000 | {"effort":"low"} | — | 204 | length | low | — | 5/1/0 | text |
...
- anthropic/claude-haiku-4-5: {"fromCase":"H1","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":200000,"output":64000},"variants":{"high":{"thinking":{"type":"enabled","budgetTokens":16000}},"max":{"thinking":{"type":"enabled","budgetTokens":31999}}},"options":{}}
- probe/unknown-model: {"fromCase":"J1","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":0,"output":0},"variants":{},"options":{}}
- openrouter/openai/gpt-5.6-terra: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1050000,"input":922000,"output":128000},"variants":{"none":{"reasoning":{"effort":"none"}},"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"high":{"reasoning":{"effort":"high"}},"xhigh":{"reasoning":{"effort":"xhigh"}},"max":{"reasoning":{"effort":"max"}}},"options":{}}
- openrouter/google/gemini-3.6-flash: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1048576,"output":65536},"variants":{"minimal":{"reasoning":{"effort":"minimal"}},"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"high":{"reasoning":{"effort":"high"}}},"options":{}}
- openrouter/google/gemini-3.1-pro-preview: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1048576,"output":65536},"variants":{"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"high":{"reasoning":{"effort":"high"}}},"options":{}}
- openrouter/deepseek/deepseek-v4-flash-0731: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1310720,"output":131072},"variants":{"low":{"reasoning":{"effort":"low"}},"high":{"reasoning":{"effort":"high"}},"max":{"reasoning":{"effort":"max"}}},"options":{}}
- openrouter/deepseek/deepseek-v4-pro: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1048576,"output":384000},"variants":{"high":{"reasoning":{"effort":"high"}},"xhigh":{"reasoning":{"effort":"xhigh"}}},"options":{}}
- openrouter/z-ai/glm-5.3: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1310720,"output":262144},"variants":{"low":{"reasoning":{"effort":"low"}},"high":{"reasoning":{"effort":"high"}},"max":{"reasoning":{"effort":"max"}}},"options":{}}
- openrouter/anthropic/claude-opus-5: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":128000},"variants":{"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"high":{"reasoning":{"effort":"high"}},"xhigh":{"reasoning":{"effort":"xhigh"}},"max":{"reasoning":{"effort":"max"}}},"options":{}}
- openrouter/anthropic/claude-sonnet-5: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":128000},"variants":{"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"high":{"reasoning":{"effort":"high"}},"xhigh":{"reasoning":{"effort":"xhigh"}},"max":{"reasoning":{"effort":"max"}}},"options":{}}
- openrouter/anthropic/claude-haiku-4.5: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":200000,"output":64000},"variants":{"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"high":{"reasoning":{"effort":"high"}}},"options":{}}
- openrouter/anthropic/claude-fable-5: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":128000},"variants":{"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"high":{"reasoning":{"effort":"high"}},"xhigh":{"reasoning":{"effort":"xhigh"}},"max":{"reasoning":{"effort":"max"}}},"options":{}}
- openrouter/x-ai/grok-4.3: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":900000},"variants":{"none":{"reasoning":{"effort":"none"}},"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"high":{"reasoning":{"effort":"high"}}},"options":{}}
- openrouter/minimax/minimax-m2.7: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":204800,"output":131072},"variants":{},"options":{}}
- openrouter/qwen/qwen3.8-27b: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":131072},"variants":{"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"xhigh":{"reasoning":{"effort":"xhigh"}}},"options":{}}
- openrouter/qwen/qwen3-coder-next: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":262144,"output":235929},"variants":{},"options":{}}
- openrouter/qwen/qwen3.6-flash: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":65536},"variants":{},"options":{}}
- openrouter/mistralai/mistral-medium-3-5: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":262144,"output":209715},"variants":{"none":{"reasoning":{"effort":"none"}},"high":{"reasoning":{"effort":"high"}}},"options":{}}
- openrouter/bytedance-seed/seed-2.0-lite: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":262144,"output":131072},"variants":{"minimal":{"reasoning":{"effort":"minimal"}},"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"high":{"reasoning":{"effort":"high"}}},"options":{}}
- openrouter/thinkingmachines/inkling: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1048576,"output":471859},"variants":{"none":{"reasoning":{"effort":"none"}},"minimal":{"reasoning":{"effort":"minimal"}},"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"high":{"reasoning":{"effort":"high"}},"max":{"reasoning":{"effort":"max"}}},"options":{}}
- openrouter/openai/gpt-4o: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":128000,"output":16384},"variants":{},"options":{}}
- anthropic/claude-opus-5: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":128000},"variants":{"low":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"low"},"medium":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"medium"},"high":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"high"},"xhigh":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"xhigh"},"max":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"max"}},"options":{}}
- anthropic/claude-sonnet-5: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":128000},"variants":{"low":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"low"},"medium":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"medium"},"high":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"high"},"xhigh":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"xhigh"},"max":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"max"}},"options":{}}
- anthropic/claude-fable-5: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":128000},"variants":{"low":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"low"},"medium":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"medium"},"high":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"high"},"xhigh":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"xhigh"},"max":{"thinking":{"type":"adaptive","display":"summarized"},"effort":"max"}},"options":{}}
- anthropic/claude-haiku-4-5-20251001: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":200000,"output":64000},"variants":{"high":{"thinking":{"type":"enabled","budgetTokens":16000}},"max":{"thinking":{"type":"enabled","budgetTokens":31999}}},"options":{}}
- anthropic/claude-sonnet-4-6: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":128000},"variants":{"low":{"thinking":{"type":"adaptive"},"effort":"low"},"medium":{"thinking":{"type":"adaptive"},"effort":"medium"},"high":{"thinking":{"type":"adaptive"},"effort":"high"},"max":{"thinking":{"type":"adaptive"},"effort":"max"}},"options":{}}
- anthropic/claude-opus-4-5: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":200000,"output":64000},"variants":{"low":{"thinking":{"type":"enabled","budgetTokens":16000},"effort":"low"},"medium":{"thinking":{"type":"enabled","budgetTokens":16000},"effort":"medium"},"high":{"thinking":{"type":"enabled","budgetTokens":16000},"effort":"high"}},"options":{}}
- openai/gpt-5.6-terra: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1050000,"input":922000,"output":128000},"variants":{"none":{"reasoningEffort":"none","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"low":{"reasoningEffort":"low","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"medium":{"reasoningEffort":"medium","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"high":{"reasoningEffort":"high","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"xhigh":{"reasoningEffort":"xhigh","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"max":{"reasoningEffort":"max","reasoningSummary":"auto","include":["reasoning.encrypted_content"]}},"options":{}}
- openai/gpt-5.6-sol-pro: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1050000,"input":922000,"output":128000},"variants":{"none":{"reasoningEffort":"none","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"low":{"reasoningEffort":"low","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"medium":{"reasoningEffort":"medium","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"high":{"reasoningEffort":"high","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"xhigh":{"reasoningEffort":"xhigh","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"max":{"reasoningEffort":"max","reasoningSummary":"auto","include":["reasoning.encrypted_content"]}},"options":{"reasoningMode":"pro"}}
- openai/gpt-5.3-codex: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":400000,"input":272000,"output":128000},"variants":{"none":{"reasoningEffort":"none","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"low":{"reasoningEffort":"low","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"medium":{"reasoningEffort":"medium","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"high":{"reasoningEffort":"high","reasoningSummary":"auto","include":["reasoning.encrypted_content"]},"xhigh":{"reasoningEffort":"xhigh","reasoningSummary":"auto","include":["reasoning.encrypted_content"]}},"options":{}}
- openai/gpt-4o: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":128000,"output":16384},"variants":{},"options":{}}
- google/gemini-3.6-flash: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1048576,"output":65536},"variants":{"minimal":{"thinkingConfig":{"includeThoughts":true,"thinkingLevel":"minimal"}},"low":{"thinkingConfig":{"includeThoughts":true,"thinkingLevel":"low"}},"medium":{"thinkingConfig":{"includeThoughts":true,"thinkingLevel":"medium"}},"high":{"thinkingConfig":{"includeThoughts":true,"thinkingLevel":"high"}}},"options":{}}
- google/gemini-3.1-pro-preview: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1048576,"output":65536},"variants":{"low":{"thinkingConfig":{"includeThoughts":true,"thinkingLevel":"low"}},"medium":{"thinkingConfig":{"includeThoughts":true,"thinkingLevel":"medium"}},"high":{"thinkingConfig":{"includeThoughts":true,"thinkingLevel":"high"}}},"options":{}}
- deepseek/deepseek-v4-pro: {"fromCase":"M0","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":384000},"variants":{"high":{"reasoningEffort":"high"},"max":{"reasoningEffort":"max"}},"options":{}}

PATCH /config per case (status, error, sandbox files the call created, /config/providers afterwards):
- M3: {"status":200,"error":null,"newFiles":["HOME/project/config.json"],"changedFiles":["HOME/.local/share/opencode/opencode.db-wal"],"sandboxFiles":["HOME/.cache/opencode/bin/ripgrep-15.1.0-x86_64-pc-windows-msvc.zip","HOME/.cache/opencode/models.json","HOME/.config/opencode/.gitignore","HOME/.local/share/opencode/log/opencode.log","HOME/.local/share/opencode/opencode.db","HOME/.local/share/opencode/opencode.db-shm","HOME/.local/share/opencode/opencode.db-wal","HOME/.local/state/opencode/locks/c9c8352b89eec0f813ad5bdfc1c1d26864d82080.lock/heartbeat","HOME/.local/state/opencode/locks/c9c8352b89eec0f813ad5bdfc1c1d26864d82080.lock/meta.json","HOME/project/config.json"],"configAfter":"{\"openrouter\":{\"options\":{\"apiKey\":\"probe-key\",\"baseURL\":\"http://127.0.0.1:59938/api/v1\"},\"models\":{\"moonshotai/kimi-k3\":{}}},\"anthropic\":{\"options\":{\"apiKey\":\"probe-key\",\"baseURL\":\"http://127.0.0.1:59938/v1\"},\"models\":{\"claude-haiku-4-5\":{\"limit\":{\"context\":200000,\"output\":24000}}}}}","after":{"anthropic/claude-haiku-4-5":{"fromCase":"M3","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":200000,"output":24000},"variants":{"high":{"thinking":{"type":"enabled","budgetTokens":16000}},"max":{"thinking":{"type":"enabled","budgetTokens":31999}}},"options":{}},"openrouter/moonshotai/kimi-k3":{"fromCase":"M3","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1048576,"output":943718},"variants":{"low":{"reasoning":{"effort":"low"}},"high":{"reasoning":{"effort":"high"}},"max":{"reasoning":{"effort":"max"}}},"options":{}}}}
- M4: {"status":200,"error":null,"newFiles":[],"changedFiles":["HOME/.local/share/opencode/opencode.db-wal","HOME/project/config.json"],"sandboxFiles":["HOME/.cache/opencode/bin/ripgrep-15.1.0-x86_64-pc-windows-msvc.zip","HOME/.cache/opencode/models.json","HOME/.config/opencode/.gitignore","HOME/.local/share/opencode/log/opencode.log","HOME/.local/share/opencode/opencode.db","HOME/.local/share/opencode/opencode.db-shm","HOME/.local/share/opencode/opencode.db-wal","HOME/.local/state/opencode/locks/c9c8352b89eec0f813ad5bdfc1c1d26864d82080.lock/heartbeat","HOME/.local/state/opencode/locks/c9c8352b89eec0f813ad5bdfc1c1d26864d82080.lock/meta.json","HOME/project/config.json"],"configAfter":"{\"openrouter\":{\"options\":{\"apiKey\":\"probe-key\",\"baseURL\":\"http://127.0.0.1:59938/api/v1\"},\"models\":{\"moonshotai/kimi-k3\":{}}}}","after":{"openrouter/moonshotai/kimi-k3":{"fromCase":"M4","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1048576,"output":943718},"variants":{"low":{"reasoning":{"effort":"low"}},"high":{"reasoning":{"effort":"high"}},"max":{"reasoning":{"effort":"max"}}},"options":{}}}}
- M8: {"status":400,"error":"{\"name\":\"BadRequest\",\"data\":{\"message\":\"Missing key\\n  at [\\\"provider\\\"][\\\"anthropic\\\"][\\\"models\\\"][\\\"claude-haiku-4-5\\\"][\\\"limit\\\"][\\\"context\\\"]\",\"kind\":\"Payload\"}}","newFiles":[],"changedFiles":["HOME/.local/share/opencode/opencode.db-wal"],"sandboxFiles":["HOME/.cache/opencode/bin/ripgrep-15.1.0-x86_64-pc-windows-msvc.zip","HOME/.cache/opencode/models.json","HOME/.config/opencode/.gitignore","HOME/.local/share/opencode/log/opencode.log","HOME/.local/share/opencode/opencode.db","HOME/.local/share/opencode/opencode.db-shm","HOME/.local/share/opencode/opencode.db-wal","HOME/.local/state/opencode/locks/c9c8352b89eec0f813ad5bdfc1c1d26864d82080.lock/heartbeat","HOME/.local/state/opencode/locks/c9c8352b89eec0f813ad5bdfc1c1d26864d82080.lock/meta.json","HOME/project/config.json"],"configAfter":"{\"anthropic\":{\"options\":{\"apiKey\":\"probe-key\",\"baseURL\":\"http://127.0.0.1:59938/v1\"},\"models\":{\"claude-haiku-4-5\":{}}}}","after":{"anthropic/claude-haiku-4-5":{"fromCase":"M8","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":200000,"output":64000},"variants":{"high":{"thinking":{"type":"enabled","budgetTokens":16000}},"max":{"thinking":{"type":"enabled","budgetTokens":31999}}},"options":{}}}}
- M11: {"status":200,"error":null,"newFiles":[],"changedFiles":["HOME/.local/share/opencode/opencode.db-wal","HOME/project/config.json"],"sandboxFiles":["HOME/.cache/opencode/bin/ripgrep-15.1.0-x86_64-pc-windows-msvc.zip","HOME/.cache/opencode/models.json","HOME/.config/opencode/.gitignore","HOME/.local/share/opencode/log/opencode.log","HOME/.local/share/opencode/opencode.db","HOME/.local/share/opencode/opencode.db-shm","HOME/.local/share/opencode/opencode.db-wal","HOME/.local/state/opencode/locks/c9c8352b89eec0f813ad5bdfc1c1d26864d82080.lock/heartbeat","HOME/.local/state/opencode/locks/c9c8352b89eec0f813ad5bdfc1c1d26864d82080.lock/meta.json","HOME/project/config.json"],"configAfter":"{\"anthropic\":{\"options\":{\"apiKey\":\"probe-key\",\"baseURL\":\"http://127.0.0.1:59938/v1\"},\"models\":{\"claude-haiku-4-5\":{\"limit\":{\"context\":200000,\"output\":24000}}}}}","after":{"anthropic/claude-haiku-4-5":{"fromCase":"M11","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":200000,"output":24000},"variants":{"high":{"thinking":{"type":"enabled","budgetTokens":16000}},"max":{"thinking":{"type":"enabled","budgetTokens":31999}}},"options":{}}}}
- M12 (wait for the catalogue to know openrouter/qwen/qwen3.8-max-0902): {"key":"openrouter/qwen/qwen3.8-max-0902","known":true,"ms":285,"polls":1,"dump":{"fromCase":"wait","keys":["id","providerID","api","name","family","capabilities","cost","limit","status","options","headers","release_date","variants"],"limit":{"context":1000000,"output":131072},"variants":{"minimal":{"reasoning":{"effort":"minimal"}},"low":{"reasoning":{"effort":"low"}},"medium":{"reasoning":{"effort":"medium"}},"high":{"reasoning":{"effort":"high"}},"xhigh":{"reasoning":{"effort":"xhigh"}}},"options":{}}}

checks: 59 matched, 0 mismatched (none), 2 recorded

That line is the output of the full-matrix run filed above, and it reconciled exactly with the table
as it then stood (59 + 2 = 61 cases). **M23 was added afterwards, by council #235 r3 wave 4, without
re-running the matrix** — the table now carries 62 cases, so a full run today scores 62 and not
61; if the 59 previously-matched rows still match it will print `60 matched, 0 mismatched (none), 2
recorded` (a prediction, not a measurement: only M23 itself was re-run). M23 is pinned on its own
instead, keyless and zero-spend, by
`tests/probe-config-only-row.integration.test.js` (`--only M23`), whose expectation is the exact line
`checks: 1 matched, 0 mismatched (none), 0 recorded — partial run (--only M23)`. Re-file the whole
line the next time the matrix is run end to end.
...
- [x] **PR 4 must fit a thinking variant UNDER the budget on direct Anthropic (#218 PR 2 council r3
  ...
  cannot hold, and add the probe row that proves it. **Done in PR 4 as the item's second clause:
  with a budget set, a declared variant of the shape `thinking: {type: 'enabled', budgetTokens: N}`
  on a model whose ceiling exceeds the budget is refused before any request (`VARIANT_OVER_BUDGET`,
  with the reservation the leg would have made — M2: 40000 — and three remedies); the probe rows
  that prove the shape (M2) and the exact fit (M17: descriptor 8000 + flag 24000 + high → 24000) are
  filed above. The exact pre-spawn fit is filed as its own item below — it needs N before the spawn
  and N is only in the post-spawn dump.**
- [ ] **Exact pre-spawn fit of a thinking variant under the budget (#218 PR 4 follow-up).** M17
  proves the shape on the wire (descriptor 8000 + flag 24000 + variant `high` → 24000, exactly the
  budget), but N is engine-derived and not a formula of the ceiling — `anthropic/claude-opus-4-5`
  declares 16000 for low, medium and high with no max, haiku 16000/31999 (M0) — and only the
  POST-spawn dump carries it. A runtime `PATCH /config` is accepted, changes nothing the engine
  serves, and writes a merged `config.json` into the engine's cwd (M3/M4/M11), so it is never a
  lever; models.dev publishes the SHAPE (`reasoning_options`: `budget_tokens` vs `effort` with
  values) but not N. The candidate design: `startServer` spawns, reads the dump for the
  (model, variant) pairs the run will send, and respawns once with `limit.output = budget − N` for
  each `enabled`-shape pair — one extra engine start, on that path only. Owner's call whether one
  rare combination is worth the second spawn.
- [ ] **Council seats have no effort knob (#218 PR 4, R6) — owner's decision.** Decision 2 (v4.5)
  rejected `thinking` on council packs as inert; PR 4 makes the lever real for solo and fanout legs
  only. The #218 Mode 2 rows (kimi 32000 reasoning / 0 output at OpenRouter's default `max`; qwen at
  `xhigh`) ARE council seats, and today's remedies for them are the budget (PR 2) and the death name
...
### Round 6 (2026-09-25) — filed, not fixed

Council round 6 (run 36150179459) reviewed the final head `be0c78bd`. The owner's rule for it: merge and cut unless a confirmed major is a defect in the 11 commits after `ad7a8e67`. None was. D3 was refuted by measurement, C2 and D2 were refuted as in earlier rounds, and the other majors predate those commits or are design. These are the items worth doing later:

- [ ] #257 (council r6 A1) — a promoted debate defence or re-vote whose relaunch answers with real, parseable text is recorded only on its superseded row (`promoted: true`); no note announces the rescue. That matches how the debate treats any defence a repair rescued (`conformance: 'repaired'` is not a degradation), but Stage 1 (`Recovered:`) and the Stage-2 judge (`judge-reasoning-only`) both announce their rescues. Candidate: an `info` note for a rescued promoted defence or re-vote.
- [ ] #257 (council r6 C1) — the judge's stand-down cause, seven values, lives only in the note's prose. Its `data` carries `attempts`, `relaunched`, `rescued` and, for R-X36/R-X49, `repairPromotedAttempt`, which do not separate every cause. Candidate: `data.standDown`, emit-when-set.
- [ ] #257 (council r6 A2, C6) — the debate round note dedupes promoted-retry markers by kind and alias, so one seat's promoted defence retry and promoted re-vote retry of the same kind collapse into one mention, and two twin seats sharing an alias into one name. Candidate: key by seat id and count the events.
